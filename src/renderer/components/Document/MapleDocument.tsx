...
const TextLine: React.FC<TextLineProps> = memo(({
  ...
  return (
    <div
      ...
      className={`group py-1 px-1 cursor-text transition-colors select-text ${
    ...
    >
      <div className="min-h-[26px]">
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
              ...
              if (sel && sel.rangeCount > 0) {
                ...
                if (rect.top > 0 && containerRect.top > 0 && rect.top - containerRect.top > 22) {
                  isAtTop = false;
                }
              }
              if (isAtTop) {
              ...
              if (sel && sel.rangeCount > 0) {
                ...
                const rect = range.getBoundingClientRect();
                const containerRect = e.currentTarget.getBoundingClientRect();
                if (rect.bottom > 0 && containerRect.bottom > 0 && containerRect.bottom - rect.bottom > 22) {
                  isAtBottom = false;
                }
              }
              if (isAtBottom) {
          ...
          className="w-full outline-none font-serif text-[17px] leading-relaxed text-[var(--text-primary)] min-h-[1.5em] select-text"
        />
      </div>
    </div>
  );
...
const MathLine: React.FC<MathLineProps> = memo(({
  ...
  return (
    <div
      data-element-id={id}
      data-element-idx={index}
      className={`group py-1.5 px-1 my-0.5 cursor-text transition-colors select-text ${
        isSelected ? 'bg-[var(--selection-bg)] maple-selected' : 'bg-transparent'
      }`}
      style={backgroundColor ? { backgroundColor } : undefined}
      onMouseDown={onRowMouseDown}
...
// ---------------------------------------------------------------------------
// MapleDocument: Continuous Word-like mathematical document workspace.
// ---------------------------------------------------------------------------
interface MapleDocumentProps {
  containerRef?: React.RefObject<HTMLDivElement | null>;
}

export const MapleDocument: React.FC<MapleDocumentProps> = ({ containerRef: externalContainerRef }) => {
  const {
    document: doc,
    activeElementId,
    setActiveElementId,
      ...
      >
        ...
        >
          ...
          >
            ...
            <div ref={elementsContainerRef} className="space-y-1 select-text">
